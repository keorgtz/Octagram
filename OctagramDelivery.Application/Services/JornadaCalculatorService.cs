...
namespace OctagramDelivery.Application.Services;
...
public class JornadaCalculatorService
{
    public TotalesJornada Calcular(DeliveryDay jornada, List<DeliveryDayCustomer> exclusiones)
    {
        ...
        foreach (var clienteId in clienteIds)
        {
            ...
            foreach (var prodId in todoDetalles.Where(d => d.CustomerId == clienteId).Select(d => d.ProductId).Distinct())
            {
        ...
        foreach (var prodId in todoDetalles.Select(d => d.ProductId).Distinct())
        {
